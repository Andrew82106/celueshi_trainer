...
Page({
    ...
    loadRecords() {
        ...
        if (user) {
            ...
            const processedRecords = records.map(r => {
                // 计算每秒点击次数 = 格子总数 / 用时
                const clicksPerSecond = (r.size * r.size) / parseFloat(r.time);
                
                return {
                    ...r,
                    formattedDate: new Date(r.date).toLocaleDateString(),
                    time: parseFloat(r.time).toFixed(2),
                    clicksPerSecond: clicksPerSecond.toFixed(2) // 保留两位小数
                };
            }).reverse();
    ...
    prepareChartData(records) {
        ...
        return records.slice(0, maxRecords).reverse().map((record, index) => {
            return {
                index: index + 1,
                value: parseFloat(record.clicksPerSecond),
    ...
    drawChart() {
        ...
        const ctx = wx.createCanvasContext('clickRateChart');
        ...
        ctx.fillText('每秒点击次数趋势', width / 2 - 60, padding.top - 5);

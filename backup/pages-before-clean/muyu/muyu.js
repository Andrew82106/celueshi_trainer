...
Page({
    ...
    playSound() {
        ...
        console.log('开始创建木鱼音频上下文...');
        const innerAudioContext = wx.createInnerAudioContext();
        innerAudioContext.autoplay = true;
        
        // 木鱼音效路径，后续需要添加实际的音效文件
        // TODO: 添加木鱼音效文件到 assets/video/muyu.mp3
        const audioSrc = '/packageZenAssets/packageZenAssets/assets/audio/muyu.wav';
        console.log('木鱼音频路径：', audioSrc);
        innerAudioContext.src = audioSrc;
        
        // 添加更多事件监听器以进行调试
        innerAudioContext.onPlay(() => {
            console.log('木鱼音频开始播放');
        });
        
        innerAudioContext.onCanplay(() => {
            console.log('木鱼音频已准备好播放');
        });
        
        innerAudioContext.onWaiting(() => {
            console.log('木鱼音频正在等待数据加载...');
        });
        ...
        innerAudioContext.onEnded(() => {
            console.log('木鱼音频播放结束');
        });
        
        innerAudioContext.onError((res) => {
            console.error('木鱼音频播放失败：', res);
            console.log('木鱼音频播放错误码：', res.errCode);
            console.log('木鱼音频播放错误信息：', res.errMsg);

...
Page({
    ...
    updateUserInfoAcceleratedFields(counts, seconds) {
        ...
        app.globalData.db.collection("userinfo").where({
            ...
            data: {
                accumulateMuyu: _.inc(0),         // 确保木鱼字段存在
                accumulateMuyuTime: _.inc(0),     // 确保木鱼时间字段存在
                accumulateSongbo: _.inc(counts),
                accumulateSongboTime: _.inc(seconds),
            ...
            updateUserLevel(app.globalData.userInfo.openId, app.globalData.db).then(updateRes => {
                console.log("颂钵页面更新用户段位成功:", updateRes);
            }).catch(err => {
                console.error("颂钵页面更新用户段位失败:", err);
            });
        }).catch(err => {
    ...
    getOnlineUserCount() {
        ...
        console.log('🔍 [颂钵页面-在线统计] 开始获取在线用户数量...');
        console.log(`⏰ [颂钵页面-在线统计] 当前时间: ${new Date(currentTime).toLocaleString()}`);
        console.log(`⏰ [颂钵页面-在线统计] 一分钟前: ${new Date(oneMinuteAgo).toLocaleString()}`);
        
        // 查询 userOnlineStatus 集合中在线的用户
        db.collection('userOnlineStatus').where({
            lastActiveTime: db.command.gt(oneMinuteAgo)
        }).count().then(res => {
            console.log(`📊 [颂钵页面-在线统计] 查询结果 - 在线用户数量: ${res.total}`);
            ...
            db.collection('userOnlineStatus').where({
                ...
                console.log(`📋 [颂钵页面-在线统计] 在线用户详细信息 (${detailRes.data.length}个):`);
                ...
                if (inconsistentUsers.length > 0) {
                    console.log(`⚠️ [颂钵页面-在线统计] 发现${inconsistentUsers.length}个用户状态不一致（活跃时间在1分钟内但状态标记为离线）`);
                }
            }).catch(err => {
                console.error('❌ [颂钵页面-在线统计] 获取详细信息失败:', err);
            });
            
        }).catch(err => {
            console.error('❌ [颂钵页面-在线统计] 获取在线用户数量失败:', err);
        });
    },
